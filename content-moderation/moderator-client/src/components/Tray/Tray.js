...
export default function Tray({ leaveCall }) {
  ...
  const addRiskScore = () => {
    const riskScores = {};
    participantIds.forEach((pax) => {
      riskScores[pax] = {
        'unauthorised_sales.drugs': Math.random(),
        'unauthorised_sales.alcohol': Math.random(),
        'unauthorised_sales.weapons': Math.random(),
      };
    });
    callObject.setMeetingSessionData({ riskScores });
  };
    ...
    <div className="tray">
      ...
      <div className="tray-buttons-container">
        ...
        <div className="actions">
          ...
          <button onClick={addRiskScore} type="button">
            <ChatIcon />
            Add Risk Score
          </button>
        </div>
